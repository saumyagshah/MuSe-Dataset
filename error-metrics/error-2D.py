...
with open('manhattan-g2o', 'r') as f:
    with open('manhattan-g2o-output-edges.txt', 'w+') as g:
        while True:
            x = f.readline()
            if not x: break
...
error = 0.0
error_abs = 0.0
#Use commented code below for computing error metric for the city10000 dataset
with open('manhattan-g2o-output-edges.txt', 'r') as f:
        while True:
            x = f.readline()
            if not x: break
            ...
            with open('manhattanOlsonGroundTruth.txt', 'r') as g:
                while True:
                    y = g.readline()
                    if not y: break
