...
using namespace std;
// ofstream myfile;
// myfile.open ("/home/saumya/muse_dataset/cartographer/info.txt", ios::out | ios::app);
...
visualization_msgs::MarkerArray MapBuilderBridge::GetTrajectoryNodeList() {
  ...
  ofstream myfile;
  myfile.open ("/home/saumya/muse_dataset/cartographer/info.txt", ios::out | ios::app);
  const auto vec = map_builder_->pose_graph()->constraints();

  // cout << "pose_graph: " << pose_graph << '\n';
  //  myfile << pose_graph()->GetTrajectoryNodePoses();
  // for (int i = 0 ; i < map_builder_->pose_graph()->constraints().size() ; i++)
  // {  
  //     map_builder_->pose_graph()->Constraint obj;
  //     obj = map_builder_->pose_graph()->constraints()[i];
  //     myfile << obj.pose.translation_weight;
  // }
  for (const auto& ele : vec) {
    myfile << "Translation weight:" << ele.pose.translation_weight <<'\n';
    myfile << "Rotation weight:" << ele.pose.translation_weight <<'\n';
    myfile << "Node Index:" << ele.node_id.node_index <<'\n';
    myfile << "Node Trajectory ID:" << ele.node_id.trajectory_id <<'\n';
    myfile << "Submap Trajectory ID:" << ele.submap_id.trajectory_id <<'\n';
    myfile << "Submap Index:" << ele.submap_id.submap_index <<'\n';
    myfile << "Translation vector:" << endl << ele.pose.zbar_ij.translation() << '\n';
    // myfile << "Quaternion vector:" << ele.pose.zbar_ij.rotation() << '\n';
    myfile << "Quaternion w:" << ele.pose.zbar_ij.rotation().w() << '\n' << "Quaternion vector:" << endl << ele.pose.zbar_ij.rotation().vec() ;
    myfile <<endl;
  }  
  ...
  for (const int trajectory_id : node_poses.trajectory_ids()) {
    // myfile << node_poses.trajectory(trajectory_id).begin()
  ...
  for (const auto& constraint : constraints) {
    // myfile << "Translation weight:" << constraint.pose.translation_weight <<'\n';
    // myfile << "Rotation weight:" << _constraint.pose.translation_weight <<'\n';
    // myfile << "Node Index:" << constraint.node_id.node_index <<'\n';
    // myfile << "Node Trajectory ID:" << constraint.node_id.trajectory_id <<'\n';
    // myfile << "Submap Trajectory ID:" << constraint.submap_id.trajectory_id <<'\n';
    // myfile << "Submap Index:" << constraint.submap_id.submap_index <<'\n';
    // myfile << "Translation vector:" << constraint.pose.zbar_ij.translation() << '\n';
    // // myfile << "Quaternion vector:" << constraint.pose.zbar_ij.rotation() << '\n';
    // myfile << "Quaternion w:" << constraint.pose.zbar_ij.rotation().w() << '\n' << "Quaternion vector:" << constraint.pose.zbar_ij.rotation().vec() << '\n';
    // myfile <<endl;
    if (constraint.tag ==
